        ...
        <Page size="A4" style={styles.page}>
            <View style={styles.header}>
                ...
                <View style={styles.invoiceDetails}>
                    ...
                    <Text style={{ fontSize: 10, color: '#666' }}>#{booking?.id?.slice(0, 8).toUpperCase()}</Text>
            ...
            <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
                <View style={styles.section}>
                    ...
                    <Text style={styles.value}>{client?.name || 'Valued Client'}</Text>
                </View>
                <View style={styles.section}>
                    <Text style={styles.label}>Destination</Text>
                    <Text style={styles.value}>{quotation?.destination || 'Global Tour'}</Text>
                </View>
            </View>
            ...
            <View style={styles.table}>
                ...
                <View style={styles.tableRow}>
                    <Text style={{ flex: 3 }}>Premium Experience Package</Text>
                    <Text style={{ flex: 1, textAlign: 'right' }}>₹{Number(booking?.amount || 0).toLocaleString()}</Text>
                </View>
                <View style={[styles.tableRow, { backgroundColor: '#fdfdfd' }]}>
                    <Text style={{ flex: 3, fontWeight: 'bold' }}>Total Paid</Text>
                    <Text style={{ flex: 1, textAlign: 'right', fontWeight: 'bold', color: '#0066FF' }}>₹{Number(booking?.amount || 0).toLocaleString()}</Text>
                </View>
            </View>
